...
def reduce_t1t2_data(data: T1T2Data) -> T1T2Data:
    """Apply Hecht-Ullman T1 T2 Analysis to reduce graph. Mutates data.

    Args:
        data (T1T2Data): Graph data (call init_t1t2 to gen from graph)
...
def init_t1t2(graph: graph_t, weights: weights_t | None=None) -> T1T2Data:
...
def recover_orig_data(data: T1T2Data) -> T1T2Data:
